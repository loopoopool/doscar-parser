...
def pbc(x):
    for i in range(len(x)):
        ...
        if abs(x[i]-1) < eps: x[i] = 0.0
    return x

def octa(x, os):
    for i, xi in enumerate(x):
        if xi - os[i] > 0.5: x[i] -= 1.0
...
class POSCAR:
	def __init__(self, filename):
		...
		self.ucMatrix = self.scalingFactor * np.array( [ extract_coord( line ) for line in data[2:5] ] )
		self.atomsLabel = split( whitespace_to_semicol( data[5] ) )
		self.atomsNumber = [ int(x) for x in split( whitespace_to_semicol( data[6] ) ) ]
		...
		self.direct = self.coorType == 'Direct' or self.coorType == 'direct' or self.coorType == 'D' or self.coorType == 'd'
		index+=1

		if self.direct:
		    self.directMatrix = np.array( [ extract_coord( line ) for line
			in data[index:index+self.ntot] ] )
		    self.cartesianMatrix = np.matmul( self.directMatrix, self.ucMatrix )
		else:
		    self.cartesianMatrix = np.array( [ extract_coord( line ) for
			line in data[index:index+self.ntot] ] )
	...
	def write_to_file(self, coordinate, filename):
		...
		labels = '   '
		for lab in self.atomsLabel: labels += lab + '  '
		data.append( labels + '\n' )
		labels = '   '
		for lab in self.atomsNumber: labels += str(lab) + '  '
		data.append( labels + '\n' )
		if self.SelectiveDynamics : data.append( 'Selective Dynamics\n' )
